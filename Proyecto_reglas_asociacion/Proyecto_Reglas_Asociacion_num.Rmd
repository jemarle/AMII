...
pacman::p_load(readr, stringr, tidyr, dplyr, readxl, ggplot2, forcats, kableExtra, greekLetters, fitdistrplus, OneR, caret, OneR, arules, arulesCBA, arulesViz, Rgraphviz, DescTools)
```
...
```{r}
extract_chi2 <- function(x, y) {
  chi2_result <- chisq.test(table(x, y))$statistic
  return(chi2_result)
}

chi2_matrix <- PairApply(discretizedQB_list$interval, 
                         FUN = extract_chi2)

cramerV_matrix <- PairApply(discretizedQB_list$interval, FUN = CramerV)

chi2_matrix
cramerV_matrix
```
```{r}
PlotCorr(chi2_matrix, main = "Matriz de Chi-Cuadrado")
PlotCorr(cramerV_matrix)
```
```{r}
# Lista para almacenar las matrices de contingencia
contingency_matrices <- list()

# Iterar sobre las variables en discretizedQB_list$interval
for (variable in columns_to_discretize) {
  # Construir la fórmula de manera dinámica
  formula <- as.formula(paste(" ~ Bankruptcy + ", variable))
  
  # Calcular la matriz de contingencia utilizando xtabs
  contingency_matrices[[variable]] <- xtabs(formula, data = discretizedQB_list$interval)
}

names(contingency_matrices) <- NULL
