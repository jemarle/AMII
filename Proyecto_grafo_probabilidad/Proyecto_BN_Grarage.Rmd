...
![Figura 1. Garage.](./data/bayesGarage.png)
...
Se mostrará en este apartado el código puesto que puede tener algo más de "miga".
...
probB_ev <- querygrain(jt_Ao, nodes = c("B"), type = "conditional")
probB_ev
```
Si el vehículo es viejo (`A = o`) vemos que la probabilidad de que haya que cambiar los frenos (`B = yes`) se ve incrementada (segundo caso frente al primero). En este caso se trata de un razonamiento causal, puesto que se trata de un tipo de razonamiento "descendente", puesto que especifica cómo la causa genera el efecto.
...
probsA_ev <- querygrain(jt_SPyBy, nodes = c("A"), type = "conditional")
probsA_ev
```
...
probA_evT <- querygrain(jt_Mh, nodes = c("A", "T"), type = "conditional")
probA_evT
```
